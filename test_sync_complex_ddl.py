...
def test_sync():
    ...
    # validate ddl
    print(f'''[{format_cur_time()}]: Test ddl of add column''')
    mysql_cursor.execute("ALTER TABLE " + TABLE_NAME + " ADD COLUMN new_col INT")
    mysql_cursor.execute("INSERT INTO " + TABLE_NAME + " VALUES(-1, 'str', 1.0, 'name', 100)")
    wait_sync(foxlake_cursor)
    validate_results(mysql_cursor, foxlake_cursor, select_query, "ddl_add_column")

    print(f'''[{format_cur_time()}]: Test ddl of drop column''')
    mysql_cursor.execute("ALTER TABLE " + TABLE_NAME + " DROP COLUMN num")
    ...
    select_query = "SELECT * FROM " + DB_NAME + "." + TABLE_NAME + " ORDER BY pk"
    mysql_cursor.execute("INSERT INTO " + TABLE_NAME + " VALUES(-5, 'str_new', 'name', 35.7)")
    wait_sync(foxlake_cursor)
    validate_results(mysql_cursor, foxlake_cursor, select_query, "ddl_rename_table")

    print(f'''[{format_cur_time()}]: Test ddl of truncate table''')
    mysql_cursor.execute("TRUNCATE " + TABLE_NAME)
    mysql_cursor.execute("INSERT INTO " + TABLE_NAME + " VALUES(-6, 'str_new', 'name', 45.2)")
    wait_sync(foxlake_cursor)
    validate_results(mysql_cursor, foxlake_cursor, select_query, "ddl_truncate_table")
