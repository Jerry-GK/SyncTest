...
FOXLAKE_PORT = 11288
FOXLAKE_STORAGE_NAME = "storage_sync_lag"
...
"""
def wait_sync(foxlake_cursor, interval: float=1, timewait=10, timeout=1800):
    begin = time.time()
    prev_applied_id = 0
...
def check_lag(mysql_cursor, foxlake_cursor, dml_start_time):
    ...
    while True:
        ...
        if mysql_res and foxlake_res and mysql_res[0] and foxlake_res[0]:
            ...
            print(f'[{format_cur_time()}]: [check_lag_thread] - Lag = {lag_seconds}s (avg = {sum(lags) / len(lags):.2f}'
                   + "s)\tQPS = " + qps.__str__())
